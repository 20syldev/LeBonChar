            ...
            <div class="form-group">
                ...
                <?php if (count($images) < 3): ?>
                    <form method="post" enctype="multipart/form-data" action="/annonce/ajouter-image?id=<?php echo $annonce['id']; ?>">
                        <label style="display:block; width:100%; max-width:400px; margin:auto; border:2px dashed #aaa; border-radius:12px; padding:40px 0; text-align:center; cursor:pointer; font-size:1.2em; color:#666;">
                            + Ajouter des images (<?php echo 3 - count($images); ?> restantes)
                    ...
                    <div style="display: flex; gap: 12px;">
                        <?php foreach ($images as $img): ?>
                            <div style="display: flex; flex-direction: column; align-items: center;">
                                <form method="post" enctype="multipart/form-data" action="/annonce/modifier-image?id=<?php echo $img['id']; ?>&annonce_id=<?php echo $annonce['id']; ?>" style="display:inline;">
                                ...
                                <form method="post" action="/annonce/supprimer-image?id=<?php echo $img['id']; ?>&annonce_id=<?php echo $annonce['id']; ?>" style="margin-top: 4px;">
                                    <button type="submit" class="btn btn-danger" title="Supprimer l'image">🗑️</button>
                                </form>
                            </div>
                        <?php endforeach; ?>
                    </div>
