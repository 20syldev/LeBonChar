...
class Annonce {
    ...
    public static function changerStatut($id, $statut) {
        $stmt = self::$pdo->prepare("UPDATE Annonce SET statut = ? WHERE id = ?");
    ...
    public static function creer($data) {
        ...
        $voiture_id = self::$pdo->lastInsertId();
    ...
    public static function ajouterAuPanier($annonce_id, $acheteur_id) {
        ...
        $stmt = self::$pdo->prepare("SELECT acheteur_id FROM Annonce WHERE id = ?");
        ...
        $stmt = self::$pdo->prepare("SELECT COUNT(*) FROM Annonce WHERE acheteur_id = ?");
        $stmt->execute([$acheteur_id]);
        if ($stmt->fetchColumn() > 0) {
            throw new Exception("Vous ne pouvez réserver qu'une seule annonce à la fois.");
        }

        $stmt = self::$pdo->prepare("UPDATE Annonce SET acheteur_id = ? WHERE id = ?");
        $stmt->execute([$acheteur_id, $annonce_id]);
    }

    public static function supprimerDuPanier($annonce_id) {
        $stmt = self::$pdo->prepare("UPDATE Annonce SET acheteur_id = NULL WHERE id = ?");
        $stmt->execute([$annonce_id]);
    }
}